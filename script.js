...
let fetchHoliday = async (country, year) => {
  ...
  if (result.meta.code == 200 && result.response != "") {
    ...
    title.setAttribute("class", "text-left heading");
    ...
    for (let i of result.response.holidays) {
      ...
      childData = `<div class='list-data'>
                         <p id="holidayy-name">Name: <span id="holiday-name">${i.name}</span></p>
                         <p id="holidayy-desc">Description: <span id="holiday-desc">${i.description}</span></p>
                         <p id="holidayy-date">Date: <span id="holiday-date">${i.date.iso}</span></p>
                         <p id="holidayy-type">Type: <span id="holiday-type">${i.type}</span></p>
                         <p id="holidayy-name">Location: <span id="holiday-desc">${i.locations}</span></p>
                         <p id="holidayy-date">URL ID: <span id="holiday-type">${i.urlid}</span></p>
                         </div>`;
